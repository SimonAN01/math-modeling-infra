...
- Typography:
  ...
  - Practical portability recommendation for English-only plots: `['Arial', 'Helvetica', 'DejaVu Sans', 'sans-serif']`.
  - Practical portability recommendation for Chinese plots: prepend a CJK stack such as `['Microsoft YaHei', 'SimHei', 'Noto Sans CJK SC', 'Source Han Sans SC']` before the Latin fallback chain.
- Size hierarchy:
...
- Minus signs:
  - Keep `axes.unicode_minus = False` so bilingual figures render negative values correctly.

## 1.1) Language-Specific Typography Rules

- Keep one visual system for both languages: same palette, linewidth, and spacing rules.
- In normal use, output one language per figure: English or Chinese.
- Switch fonts only to gain glyph coverage, not to create a separate style for Chinese figures.
- For mixed labels such as `Accuracy / 准确率`, prefer short slash-separated or parenthetical forms instead of full translated sentences, and use them only when explicitly requested.
- Avoid overly decorative CJK fonts. Use clean sans-serif fonts so Chinese labels do not dominate the figure visually.
- If a machine lacks the preferred Chinese fonts, rely on fallback order instead of hard-coding a single font.
